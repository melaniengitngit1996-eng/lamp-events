<template>
    <div class="row justify-content-center my-4">
        <div v-bind:class="{ 'col-md-8 col-sm-12 mb-4': isRebooking, 'col-md-6 col-lg-4 mb-4': !isRebooking }"
            v-for="(registration, i) in registrations" :key="i">
                ...
                <div slot="header" class="clearfix"
                    ...
                    <div style="width: 30px; float: right;">
                        <el-button icon="el-icon-download"
                            class="block el-button el-button--primary float-end is-plain md:hidden mx-0 p-1 sm:hidden xs:hidden"
                            type="primary" plain @click.preventDefault="printThis(`capture_${i}`)" />
                    </div>
                </div>
                <div>
                    <div class="row">
                        <div class="col-sm-12 col-md-12 col-lg-12 col-xl-6 div-personal-details">
                            <div class="row mb-3">
                                <div class="col-md-12">
                                    <small>Name</small>
                                    <span class="text-lg font-bold d-block text-uppercase text-break">{{
                            ...
                            <div class="row mb-3">
                                <div class="col-md-12">
                                    ...
                                    <span class="text-lg font-bold d-block text-uppercase text-break">{{
                                        registration.facebook_name || '--' }}</span>
                                </div>
                            </div>
                            <div class="row mb-3">
                                <div class="col-md-12">
                        ...
                        <div v-if="registration.booked_dates.length > 0" class="el-col el-col-md-16 el-col-12 mb-3">
                            ...
                            <span class="text-md font-bold d-block">
                                <el-tag size="mini" effect="dark"
                                    :type="registration.booking_status === 'Confirmed' ? 'success' : (registration.booking_status === 'Cancelled' ? 'danger' : 'warning')">{{
                                        registration.booking_status }}</el-tag>
                            </span>
                        </div>
                    </div>
                    <div class="row">
                    ...
                    <div v-if="registration.attending_option != 'Online' && event.with_booking" class="row mb-3">
                        <div class="col-md-12">
                            ...
                            <span class="text-md font-bold"
                                v-if="registration.booked_dates.length > 0 && !event.has_multiple_venues"
                                v-html="registration.booked_dates.join([separator = ',  '])"></span>

                            <el-row :gutter="5" class="text-md font-bold" style="margin-bottom: -5px"
                                v-else-if="registration.booked_dates.length > 0 && event.has_multiple_venues">
                                <el-col v-for="booked in registration.booked_dates" :key="booked.event_date" :xs="8"
                                    :sm="8" :md="8" :lg="6" :xl="6" style="margin-bottom: 5px">
                                    <div class="el-tag custom-height">
                                        <div style="display: inline-block !important; color: #409EFF">{{
                                            booked.event_date }}</div>
                                        <div style="display: inline-block !important;">{{ booked.venue }}</div>
                                    </div>
                                </el-col>
                            </el-row>
                            <small class="font-bold text-black-50 text-md" v-else>Not yet booked. Please reach out to
                                your local coordinator to book your schedule.</small>
                        </div>
                    </div>

                    <hr v-if="registration.attending_option != 'Online'"
                        style="margin: 1rem -20px; border-color: gray;" />
                    ...
                    <div v-if="registration.attending_option != 'Online'" class="row mt-2">
                        <div class="col-md-12">
                            <small style="
                                ...
                                display: flex;
                            ">*** Please screenshot this ticket. This will be your virtual LAMP ID number, in case you
                                opted not to avail the physical card. This will be used in the future LAMP church events
    ...
    methods: {
        ...
        open() {
            ...
            if ((this.registrations[0].registration_type === 'Member' || this.event.slug == 7382159075) && this.registrations[0].attending_option != 'Online' && this.registrations[0].rate > 0) {
                ...
                else if (this.event.slug == 1226292026) {
                    var payment_due_date = this.event.payment_due_date;
                    if (this.registrations[0].attending_option == 'Local Church' || this.registrations[0].attending_option == 'Cluster Based Satellite') {
